...
class App(customtkinter.CTk):
    ...
    def btn_convertir_c_f_on_click(self):
        ...
        temp_c_to_temp_f = (temp_c_float * 9/5) + 32
        temp_c_to_temp_f_float = float(temp_c_to_temp_f)

        calc_info = "({0} °C x 9/5) + 32 = {1} °F".format(temp_c_float, temp_c_to_temp_f_float)

        alert(title="", message=calc_info)

    def btn_convertir_f_c_on_click(self):
        temp_f = self.txt_temperatura_f.get()
        temp_f_float = float(temp_f)

        temp_f_to_temp_c = (temp_f_float - 32) * 5/9
        temp_f_to_temp_c_float = float(temp_f_to_temp_c)

        calc_info = "({0} °F − 32) × 5/9 = {1} °C".format(temp_f_float, temp_f_to_temp_c_float)
